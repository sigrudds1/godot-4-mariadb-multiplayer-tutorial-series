...
static func tls_client_connect(p_tcp_peer: StreamPeerTCP,
		p_url:String = "",
		p_tls_opt: TLSOptions = null,
		p_timeout: int = 2000) -> StreamPeerTLS:
	var tls_peer: StreamPeerTLS = StreamPeerTLS.new()
	var err: int = tls_peer.connect_to_stream(p_tcp_peer, p_url, p_tls_opt)
	if err != OK:
		printerr("TLS connection error:%s"% err)
		tls_peer = null
		return tls_peer
...
static func tcp_connect(p_url: String, p_port: int, p_timeout: int = 2000) -> StreamPeerTCP:
	...
	if err:
		printerr("TCP connection error:%d host:%s port:%d" % [err, p_url, p_port])
	...
	if (err || tcp_peer.get_status() == StreamPeerTCP.STATUS_CONNECTING ||
		...
		printerr("Cannot connect to host:%s port:%d" % [p_url, p_port])
