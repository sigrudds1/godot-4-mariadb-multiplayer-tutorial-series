...
sudo mariadb-dump --user=root -p --no-data --lock-tables --databases GameSrvrTemplate > GameSrvrTemplate_no_data_dump_$(date +'%Y%m%d_%H%M%S').sql
sudo mariadb-dump --user=root -p --no-create-info --lock-tables --databases GameSrvrTemplate > GameSrvrTemplate_data_$(date +'%Y%m%d_%H%M%S').sql