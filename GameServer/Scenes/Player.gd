...
var _msg_blocked_plyrs: PackedStringArray = []
...
func add_msg_blocked_plyr(p_plyr_dname: String) -> void:
	...
	if not Sanitize.check_display_name(p_plyr_dname): return
	
	if _msg_blocked_plyrs.find(p_plyr_dname) == -1:
		if not _msg_blocked_plyrs.push_back(p_plyr_dname): pass


func remove_msg_blocked_plyr(p_plyr_dname: String) -> void:
	var idx: int = _msg_blocked_plyrs.find(p_plyr_dname)
	if idx > -1: _msg_blocked_plyrs.remove_at(idx)
...
func _send_msg(p_from_plyr: String, p_msg: String) -> void:
	if _msg_blocked_plyrs.find(p_from_plyr) > -1: return
...
func _update_db_blocked_plyrs(p_thread: Thread) -> void:
	...
	for plyr_dname:String in _db_msg_blocked_plyrs:
		if _msg_blocked_plyrs.find(plyr_dname) == -1:
	...
	for plyr_dname: String in _msg_blocked_plyrs:
